    ...
    Draw(hasFocus: boolean): void {
        this._state.asActivity(v => {
            ...
            ADrawTextButton(this._activity_state, (v => {
                ...
                let result = v.slice(0, 3).map(a => ActivityDictionaryText(`Activity${a}`)).join(", ");
                if (v.length > 3) {
                    result += GetText(" and {0} more", [v.length - 3]);
                }
                return result;
            ...
            ADrawTextButton(this._bodypart_state, (v => {
                ...
                let result = v.slice(0, 3).map(a => {
                    let group = AssetGroupMap.get(a);
                    return group ? group.Description : a;
                }).join(", ");
                if (v.length > 3) {
                    result += GetText(" and {0} more", [v.length - 3]);
                }
