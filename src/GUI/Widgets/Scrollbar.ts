...
export class Scrollbar extends AGUIItem {
    ...
    get offset() {
        ...
        return Math.floor(this.scrollbar_bar_offset / this.step);
    }
    ...
    scroll_up() {
        this.scrollbar_bar_offset = Math.max(0, this.scrollbar_bar_offset - this.step);
    }

    scroll_down() {
        this.scrollbar_bar_offset = Math.min(this.scrollbar_bar_max_offset, this.scrollbar_bar_offset + this.step);
    }
    ...
    Click(mouse: IPoint): void {
        if (WithinRect(mouse, this.scrollbar_up)) this.scroll_up();
            ...
            this.scrollbar_bar_offset = Math.max(0, Math.min(this.scrollbar_bar_max_offset, expected_offset));
        }
    }
